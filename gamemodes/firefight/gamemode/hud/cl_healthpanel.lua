...
local HealthPanel = {}
local size = {w = ScrW()*.4, h = ScrH()*.1}
local pos = {x = ScrW()/2 - size.w/2, y = ScrH()*.1}
...
function HealthPanel:DrawShields(w, h)
    ...
    surface.SetDrawColor( 50, 180, 220, 100 )
...
function HealthPanel:DrawHealth(w, h)
    if (LocalPlayer()==nil) then return end
    --Draw the fill
    local healthPerc = (LocalPlayer():Health()/LocalPlayer():GetMaxHealth()) --This doesn't seem to work unless we have 0.7 or lower... why?
    surface.SetDrawColor( 50, 180, 220, 100 )
    surface.SetAlphaMultiplier(0.75)
